...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$deadline=(Get-Date).AddSeconds(65); do { try { $response=Invoke-WebRequest -UseBasicParsing -Uri 'http://127.0.0.1:28765/v3' -TimeoutSec 2; if ($response.StatusCode -eq 200) { Start-Process 'http://127.0.0.1:28765/v3'; exit 0 } } catch {}; Start-Sleep -Milliseconds 500 } while ((Get-Date) -lt $deadline); Write-Error 'Unified Workbench Service did not become ready within 65 seconds.'; exit 1"
