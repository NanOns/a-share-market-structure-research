...
| next_stage | 先设计并验收 accepted anchor/outcome revision 可见性，再实现 same-day writer 与 ordered replay；完成后另用临时 PostgreSQL schema 通过 D1→D2→D3 合成链。之后实现 PIT-safe RPS20 provider，或继续将其保持为明确的独立 provider gap；真实停牌复牌、连续交易日和下一 accepted day 的 Path State V2 落库仍按未来样本门推进。 |
...
- SOURCE_MODEL_BOUNDARY context 现在允许同 source family/entity 的 selection contract 变化，保留旧 episode 首日事实并采用当前来源 context；定向 context 测试通过。生产 writer/数据库端到端验收仍未完成。
- 小体积 9/24 Focus、technical identity 与 outcome 诊断摘要已经归档到 `docs/evidence/`；Auto Apply 关闭状态也包含在 core receipt。

## 尚未通过的审计门

同日 revision writer 和按交易日顺序 replay writer 尚未实现。现有数据库模型没有按 accepted Focus revision 限定 anchor/outcome 可见性的 head；只取消 writer 的 revision=1 限制会让旧 revision 的 anchor/outcome 继续参与结算，因此该改法不安全。独立跟踪项及所需 schema/事务验收见 `docs/audits/FOCUS_SAME_DAY_REVISION_ORDERED_REPLAY_GAP_20260924.md`。
