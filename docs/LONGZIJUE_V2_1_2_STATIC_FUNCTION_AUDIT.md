...
可借鉴：将工作台首页改为“市场概况 → 强势板块 → 板块梯队 → 代表个股”的渐进式视图；定时刷新在本项目只能刷新本地已发布快照，不能接在线行情。
...
可借鉴：用离线数据定义“主线”为版本化描述规则，例如近 N 日进入板块强度前列的次数、连续性、上涨宽度、成交额扩张、强势成员保留率，而不是简单累计涨幅。DIY 挖掘可以改为合同化筛选器，不允许用户任意生成不可审计指标。
...
可借鉴：当前工作台已经有板块—个股联动，可增加“选择 2～4 个板块求交集”“板块内筛选五类结构”“成员留存率”“板块内排名变化”。炒作原因无法从纯行情可靠推导，只能标为外部资讯缺口或由用户手工维护。
...
可借鉴：当前项目禁止在线数据，因此只能做“本地公司行为日历”（除权除息、停复牌等可从本地事件文件确认的内容）和“用户导入事件日历”。不能把未知在线事件偷偷混入离线扫描结果。
...
可借鉴：如果没有本地龙虎榜数据源，工作台只能预留插件接口或接受用户导入文件。成交额、换手和异常放量可离线计算，但不能冒充龙虎榜席位数据。
...
可借鉴：离线环境不能复制“用户关注热度”，但可创建明确命名的“行情热度代理”：成交额百分位、换手变化、板块成员扩散、进入五类队列数量等，必须与互联网热榜概念区分。
...
可借鉴：当前离线工作台不需要复制会员体系。若未来多用户部署，可以借鉴“能力开关＋服务端配置＋清晰失效提示”，但研究算法本身不应因账号等级改变。

## 4. 数据来源与用途

| 来源 | 静态证据中的域名/接口 | 主要用途 | 离线可替代程度 |
|---|---|---|---|
| 选股宝 | `flash-api.xuangubao.cn`、`flash-api.xuangubao.com.cn` | 涨停/炸板/跌停池、题材与成员、市场情绪线 | 行情池可部分替代；题材原因和实时状态不可完整替代 |
| 同花顺 | `data.10jqka.com.cn`、`dq.10jqka.com.cn`、`ozone.10jqka.com.cn`、`quota-h.10jqka.com.cn`、`comment.10jqka.com.cn` | 市场概览、涨停分析、热榜、游资、指数 K 线、投资日历 | 日线统计可替代；热榜、原因、席位和事件资讯不可替代 |
| 东方财富 | `push2ex.eastmoney.com`、`datacenter-web.eastmoney.com` | 资金流、投资日历 | 成交量价可部分替代；专有资金流和事件需外部数据 |
| 龙虎榜/开盘啦类站点 | `longhuvip.com` 多个子域 | 创新高、板块、成员、炒作原因 | 创新高可替代；原因文本不可替代 |
| 新浪/搜狐/腾讯行情 | `hq.sinajs.cn`、`finance.sina.com.cn`、`q.stock.sohu.com`、`qt.gtimg.cn` | 个股报价、涨幅、历史行情补充 | 通达信本地日线可替代非实时部分 |
| 财联社 | `cls.cn/api/calendar` | 投资日历 | 只能由本地事件或人工导入部分替代 |
| 软件自有服务 | `www.loong1.com/prod-api` | 登录、会员、配置、反馈、部分市场/复盘服务 | 不应迁移；工作台使用本地合同和配置 |

注意：发现接口地址只证明程序包含调用路径，不代表接口公开授权、当前可用或适合复制。当前项目守则禁止接入网络行情，因此借鉴应集中于产品结构和可由本地数据重算的逻辑。
...
这些功能可以预留“用户导入或未来合规插件”接口，但不能由价格数据冒充。
