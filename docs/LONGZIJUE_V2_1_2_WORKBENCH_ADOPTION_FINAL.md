# 龙字诀 v2.1.2 功能审计与工作台采纳方案（最终取舍版）

> 2026-09-09升级设计复审：功能取舍仍作为产品背景；实施以[M7–M15完整实施方案v2.1](WORKBENCH_M7_M15_IMPLEMENTATION_PLAN_V2_1.md)为准，后续见[工作台后续发展计划](WORKBENCH_POST_M15_ROADMAP.md)。2.1已独立包含审计、接口、数据库、UI与验收细节。本报告的模块/接口清单属于静态证据，不证明远端算法完整还原或接口目前免费可用。
...
需要注意：仓库根目录 `AGENTS.md` 当前仍写有“No network data”。本文依照用户最新产品要求重新评估在线能力；真正开始接入网络数据前，应同步修改这条现行实施守则。
...
2. 板块周期矩阵：近 5/10/20/30 日的强度、排名、上涨宽度和成交额变化。
3. 板块成员留存与龙头更替：强势成员留存率、成员扩散/收缩、首位股票变化。
4. 主线周期：近 N 日强度上榜次数、连续性、宽度、成交额扩张和强势成员保留率；在线题材热度、涨停原因和新闻只作独立增强证据。
5. 当前板块—个股联动加强：2～4 个板块交集、板块内五类结构筛选、成员留存率和排名变化。
6. 只读板块属性库：行业/概念/风格的交集、并集、行情标签排除和成员变化审计。
7. 创新高/RPS：20/30/60/100 日创新高、连续创新高、板块创新高宽度和趋势曲线。
8. 市场情绪历史：上涨/下跌家数、涨停/跌停数量、成交额、创新高数量和五类队列数量。
9. 优先发挥本地成交额、换手率、异常放量和 MA 均线的组合分析价值。

### 在线增强：只测试 A、B 两级来源

- A 级：有公开文档或明确许可的官方免费接口。
- B 级：浏览器公开可访问、无需绕过登录或私有鉴权的网页数据接口；仅用于个人研究，必须低频缓存、失败降级和字段监控。
- 不测试 C 级动态签名/Cookie 绕过接口，也不复用 D 级龙字诀私有、会员或鉴权接口。
- 优先验证同花顺和东方财富公开热榜；可用时作为“平台关注热度”接入。
- 深入分析涨停原因的数据来源；只有找到 A/B 级可用来源才接入，否则放弃该功能。
- 龙虎榜优先级较低；只有低成本 A/B 级来源成熟时再接入。

### 暂时不做

- 精确实时炸板时间；当前没有需求。
- 未来政策/会议导向的在线投资日历；项目当前以已经发生的数据分析为主。
- 每日研究快照导出包；目前单人本地使用，没有必要。
- 跳转通达信或其他终端；后续更适合使用东财或腾讯数据自行绘制 K 线。
- 改造现有分页、服务器缓存、原子写入和发布身份机制。
- 工作台登录、会员、积分和自动更新系统。
- AI 生成复盘暂不进入近期范围。

涨停数量本身可以依据本地日线和证券涨跌停规则计算；用户当前不要求精确盘中封板/炸板时间。涨停原因是另一类语义数据，只有在线 A/B 级来源验证通过才展示。
...
本轮采纳：本地实现市场广度、成交额和历史统计；涨跌停/连板需要日期敏感规则与可靠参考价，资料不足时分开显示近似和未知。日线高价触板不等于盘中曾封板，不能据此冒充精确炸板；实时炸板不在当前需求。市场周期展示可复核分项，不照搬未还原的黑盒温度分。
...
本轮不采纳投资日历或未来事件驱动页面。公司行为的日期数据仅作为复权、股本和参考价计算所需基础资料，不能因此扩大为新的日历产品功能。
...
本轮采纳：可用本地行情补算创新高池、连续新高、板块新高宽度和曲线，但不能直接由DIST_HIGH或POS推出全部新高规则。现有RS是收益差，RPS是另算的截面百分位；收盘创新高与盘中最高价须分清。20/30/60/100日完整窗口、逐窗口连续字段和历史资料口径见审计修订版。不对未经实测的追溯性作优于原软件的承诺。
...
本轮不采纳AI复盘和每日研究导出包，保持本地单人使用定位；本节仅记录原软件功能。
...
本轮不采纳外部终端跳转。个股在工作台内部打开透视和本地K线；有需求且来源验证通过后再增强盘中图表。
...
对当前工作台最有价值的是“内部统一模型、多板块交集、跨日留存、真实数值排序、原因与量价分离”。在线热榜可作为独立平台关注证据展示，但不直接改写本地结构分类。
...
1. 本地收盘涨停梯队与连板统计；当前不建设精确盘中炸板时间能力。
2. “行情热度代理”，使用成交额、换手、宽度和队列覆盖，不能叫互联网热榜。
3. 主线周期描述规则。
4. 板块趋势曲线、市场周期和历史详情弹窗。
...
- C、D 级仅用于排除：依赖 Cookie/动态签名绕过，或属于龙字诀私有鉴权/会员服务的来源，不进行接入测试。

第一批验证顺序建议：东方财富公开热榜/行情接口、交易所或官方披露数据、同花顺公开热榜；随后再评估公开涨停原因，龙虎榜放在末位。每个适配器先做只读样本验证和字段合同，不能直接写进正式扫描链路。

## 8.2 推荐的融合架构

1. `local_core`：通达信价格、成交量、板块成员和全部结构因子。
2. `online_market`：经验证的盘中价格、公开热榜和市场概况增强。
3. `online_semantic`：涨停原因、题材信息、龙虎榜和平台热榜；投资日历不在当前范围。
...
即使不登录，也能从客户端代码确认相当多的会员设计，因为界面、权限检查和业务入口都在本地包中。不能静态确认的主要是服务端当下返回的套餐价格和动态模块清单。

### 明确列出的 VIP 功能

客户端内置文案明确写出：**天眼模式、梯队周期、智库、主线周期、题材周期、大盘周期**。主窗口还会从服务端动态获取 VIP 模块名，并在对应页签前增加星号，因此实际会员页签可能随服务端配置调整。

### 权限控制流程

1. `VipModuleChecker` 从接口获取 VIP 模块列表，缓存后按页签名称判断是否为会员模块。
2. 用户切换页签时，`MainWindow.on_tab_changed` 先检查该页是否为 VIP 模块。
3. `VipInfoFetcher` 请求 `/getVipInfo`，读取 `vipStatus`、`vipExpireTime`、`vipPermissions` 等字段，并缓存 5 分钟以减少卡顿。
4. 状态包括普通用户、有效 VIP、永久会员和过期会员；模块还可按 `vipPermissions` 做细粒度判断。
5. 无权限时恢复到先前页签，并显示“暂无访问权限”或增值服务提示；统一 `ensure_vip` 返回 False 后中断业务逻辑。
6. 未登录用户先进入微信扫码登录；登录成功后再次检查权限。401 且请求带授权头时触发登录失效处理并清空登录信息。
7. 某些功能不是整页锁定，而是局部增强锁定，例如概念库存在普通详情加载与 `forced_vip` 强制刷新两条路径。

### 登录和状态模型

- 微信二维码创建会话，轮询扫码和确认状态，超时时间约 2 分钟。
- Token、微信 OpenID 和用户信息保存到本地配置；退出登录时清理。
- 另有设备硬件 ID 鉴权接口 `/member/user/deviceAuth`，会员模型后台定期刷新并通知界面。
- VIP 信息和模块清单均有单例缓存，避免每次切页发起网络请求。

### 购买、续费与积分

- `VipManager` 拉取套餐列表，构建套餐卡片，创建订单并显示支付二维码，然后轮询订单状态。
- 已是会员时不重复弹购买；过期会员进入续费；无 Token 时先扫码登录再进入支付。
- 支付成功后刷新会员模型和界面权限。
- 邀请一位成功开通 VIP 的好友可获得 1 积分；代码文案显示 1 积分可兑换 7 天 VIP。
- 存在积分兑换对话框、邀请链接/邀请码复制和套餐折扣类型解析。

### 静态分析无法确认的部分

- 当前套餐价格、期限、折扣和支付渠道返回值。
- 服务端此刻下发的完整 VIP 模块列表。
- 免费用户是否有试用次数，以及各模块是否存在每日限次。
- 服务端可能进行的二次权限判断。

客户端可见的会员检查流程可以描述；服务端权限、收费配置和分析算法不能仅凭这些静态证据完整还原。本项目暂不实现登录、会员、积分和更新系统。

## 9.2 六项会员功能的具体借鉴方式

本项目不复制龙字诀的登录、付费或权限体系，只借鉴会员模块背后的研究组织方法。

| 龙字诀会员功能 | 可借鉴的核心 | 当前工作台落点 | 数据边界 |
|---|---|---|---|
| 天眼模式 | 点击股票后跨来源聚合近期题材、异动和逻辑 | 改造为“个股研究透视”抽屉：结构、强势关联板块、MA、成交额、换手、异常放量、新高、板块周期及在线增强证据 | 本地指标为基础；在线原因只接 A/B 级来源 |
| 梯队周期 | 当日涨停、昨日涨停、连板高度、晋级和断板的跨日分层 | “市场周期/梯队”子页，先实现收盘涨停梯队，不要求精确盘中炸板时间 | 本地日线按证券涨跌停规则识别；在线数据只作校验或语义补充 |
| 智库 | 概念资料、成员股、多概念交叉选择、股票逻辑聚合 | 建立只读板块属性库和交叉筛选器，支持交集、并集、排除标签、成员变化 | 通达信行业/概念/风格为基础；行情/事件标签隔离 |
| 主线周期 | 多日题材持续性、成员变化、资金和代表股聚合 | 建立版本化主线状态：持续、新晋、再加速、扩散、高位收缩、退潮、数据不足 | 本地板块时间序列决定分类；在线热度/原因不改变首版分类 |
| 题材周期 | 题材×交易日矩阵、Top-N、高亮、趋势曲线 | 板块周期矩阵，展示5/10/20/30日排名、宽度、成交额、成员留存和龙头更替 | 必须先建设历史派生层和成员快照口径 |
| 大盘周期 | 市场广度、涨跌停、成交额、指数和情绪历史对齐 | 市场周期页，拆成市场广度、涨停强度、创新高强度、成交活跃度、趋势覆盖率 | 优先本地计算；不复制在线黑盒“市场温度” |

六项功能的实现依赖已在复审中修正：范围与板块语义 → 历史分片/身份 → 技术历史与历史结构 → 板块矩阵 → 主线/交叉筛选 → 个股透视。市场技术历史可在技术层完成后并行，梯队依赖日期敏感参考价，在线增强按来源独立推进。完整设计见[《M7–M15完整实施方案v2.1》](WORKBENCH_M7_M15_IMPLEMENTATION_PLAN_V2_1.md)。
