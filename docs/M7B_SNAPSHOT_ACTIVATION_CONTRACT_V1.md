...
快照主记录和全部 entries 在同一事务中准备。已有 `(snapshot_id, domain, trade_date)` 必须逐项核对为相同 `slice_id`，冲突或额外条目立即拒绝；`ON CONFLICT` 不得掩盖内容差异。
...
读取 API 必须解析成功 publication 的实际绑定并返回 `analysis_snapshot_id` 与 `AVAILABLE`；旧发布没有绑定时才返回 `NOT_BUILT`。
