...
每条输入记录包含角色、相对路径、文件 SHA-256（目录引用则包含 source bundle SHA-256 与 entry_count）、`observed_at`、`effective_date` 和日期范围。当前成员快照明确标记 `CURRENT_SNAPSHOT_ONLY`、`pit_safe=false`；它可固定成员回算，但不能被描述为历史 PIT 成员序列。
...
`verify_source_manifest` 校验清单自身哈希、工作区路径边界、每个文件 SHA-256、目录存在性和 entry_count。输入被修改、删除、路径越界或清单字段不完整时失败；冻结步骤不做历史计算、不写分析表、不改变发布头。
