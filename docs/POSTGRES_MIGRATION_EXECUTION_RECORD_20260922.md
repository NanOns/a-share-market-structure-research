...
随后建立 `PostgresPublicationWriter`：提供 publication identity、按交易日计算 revision、显式允许的日表批量写入、artifact 登记、observation/outcome identity、publication head 绑定和成功状态更新，所有操作在同一 PostgreSQL transaction 中提交；`scripts/pg_publication_writer_rehearsal.py` 验证 publication round-trip、stock 日表批量行、artifact、outcome 幂等、head 绑定、强制回滚和清理，结果为 `DEGRADED_PASS_PUBLICATION_WRITER`，报告位于 `runtime/postgres_migration/20260922/pg_publication_writer_rehearsal_report.json`。该 writer 仍未注入 `OneClickPublisher` 的主 transaction 和 PG repository factory，publisher 入口继续保持 `NOT_MIGRATED`。
...
本轮完成 publisher 的显式 PostgreSQL 主流程接入（仍不改变默认线上后端）：新增 `PostgresPublicationBackendFactory`，将 `PostgresPublicationWriter`、`PostgresHistoryJobRepository`、`PostgresRelationRepository` 组合为同一连接/事务 backend；`OneClickPublisher.submit/status/run` 在显式注入该 factory 时分别使用 PG job state、status projection、publication/bulk/observation/outcome/head 提交和 relation binding。新增 `scripts/pg_publisher_end_to_end_rehearsal.py`，在真实 PostgreSQL 上验证 publisher run、刷新实例后的 status projection、publication/stock/observation/outcome/head 可见性，随后恢复或删除测试 head 并清理所有探针行；结果为 `DEGRADED_PASS_PG_PUBLISHER_END_TO_END`，报告位于 `runtime/postgres_migration/20260922/pg_publisher_end_to_end_rehearsal_report.json`。该探针未触发生成、未切换线上配置。Publisher 入口的 inventory 证据已更新，但仍为 `NOT_MIGRATED`，因为 app 默认 factory、主 API 和其余 11 个入口尚未在维护窗口切换。

随后完成 HISTORY_ANALYSIS 服务的 PG 端到端边界验证：`HistoryJobService` 通过已有 `HistoryJobRepository` 合同显式注入 `PostgresHistoryJobRepository` 后，任务校验、幂等提交、worker 批次、取消边界、事件投影和清理均在 PostgreSQL 上执行。`scripts/pg_history_job_service_rehearsal.py` 使用已迁移 publication/slice 做隔离探针，覆盖成功任务、重复提交复用、批次边界取消和 `CANCELLED` 最新事件，结果为 `DEGRADED_PASS_PG_HISTORY_JOB_SERVICE`，报告位于 `runtime/postgres_migration/20260922/pg_history_job_service_rehearsal_report.json`。探针只写入并清理 jobs/attempts/events，不生成研究数据；`app.py` 默认实例仍是 DuckDB，因此该入口仍记录为 `NOT_MIGRATED`。

本轮继续完成历史分析激活入口的 PostgreSQL 事务边界：新增 `PostgresAnalysisActivationRepository`，在受控 schema search path 下将激活服务的参数占位符和事务命令映射到 PostgreSQL；`AnalysisActivationService.prepare/activate` 的 snapshot、entries、publication clone、analysis binding、publication head 和 job event 仍由同一外部事务控制。`scripts/pg_analysis_activation_rehearsal.py` 使用已迁移 publication/slice 做探针，验证 prepare、activate、重复激活复用、snapshot 成功、publication-analysis 绑定、head 前进及 head 恢复/全量清理，结果为 `DEGRADED_PASS_PG_ANALYSIS_ACTIVATION`，报告位于 `runtime/postgres_migration/20260922/pg_analysis_activation_rehearsal_report.json`。未生成研究数据，`app.py` 默认激活服务仍使用 DuckDB，入口继续为 `NOT_MIGRATED`。

随后完成运维备份入口的 catalog 注入验证：`BackupService` 已支持显式注入 `BackupCatalogRepository`，本轮用 schema-only DuckDB 探针文件和真实 `PostgresBackupCatalogRepository` 验证 catalog round-trip、目录读取、restore drill 哈希/结构校验及清理；报告 `runtime/postgres_migration/20260922/pg_backup_service_catalog_rehearsal_report.json` 的结果为 `DEGRADED_PASS_PG_BACKUP_SERVICE_CATALOG`。该探针未复制生产数据库、未写入真实备份目录，物理 DuckDB 备份仍要求维护窗口，`backup.py` 默认实例仍保持 DuckDB。

本轮继续完成存储治理入口的 PostgreSQL 元数据注入：`StorageGovernance` 通过 `PostgresWriteRepository` 执行对象登记、租约获取/释放、cleanup plan 写入和元数据投影；`scripts/pg_storage_governance_rehearsal.py` 验证登记 round-trip、lease 生命周期、cleanup plan、payload projection 和清理，结果为 `DEGRADED_PASS_PG_STORAGE_GOVERNANCE`。探针只写入并清理唯一的 `storage_objects/leases/cleanup_jobs` 记录，不触碰文件删除和 TDX 根；文件 quarantine/delete 仍 fail-closed，线上默认仍是 DuckDB。
...
| 字段 | 值 |
|---|---|
| stage | `PGM-00/01/02/03/04/05-shadow/06-shadow/07-drill/08-rehearsal/09-inventory/09-adapter-slices/09-publisher-pg-e2e/09-history-job-pg-e2e/09-analysis-activation-pg-e2e/09-backup-catalog-pg-e2e/09-storage-governance-pg-e2e` |
| stage_contract | `POSTGRES_MIGRATION_EXECUTION_RECORD_V1` |
| evidence | 目标连接、冻结快照 SHA-256、103 表复制、逐表行数对账、服务恢复、API shadow、PG 备份恢复、PGM-08 隔离切换演练、当前 12 个应用直连点切换盘点、TodayResearchBundleReader/SourceFreezer/ResultObjectRepository/SliceRepository/OperationsMetadataReader/ConfigVersionStore 适配器边界、TurnoverEnrichmentService 内存 Parquet 分类证据、publication head shadow、研究元数据 shadow、研究状态/成员角色原始行 shadow、关系边 fallback shadow、适配器注入与配置回退隔离演练、统一只读 repository 合同 shadow、operations 写入边界幂等/事务回滚演练、research run 写入合同幂等/事务回滚演练、ArtifactCatalog managed-root/路径回退演练、隔离 adapter injection 双读与 PG 503 fail-closed 演练、ConfigVersionStore PG 写入/读取/清理演练、StorageMetadataRepository PG 登记/幂等/跨事务读取/租约/清理计划演练、StorageGovernance PG register/lease/cleanup-plan/清理演练、ResultObject/SliceRepository PG 幂等与回滚演练、PostgresHistoryJobRepository job/attempt/event/回滚演练、HistoryJobService PG submit/幂等/cancel/event projection/清理演练、AnalysisActivationService PG prepare/activate/idempotent/head-restore/清理演练、BackupService PG catalog/restore-drill/清理演练、BackupCatalogRepository 目录 round-trip/重放/回滚演练、PostgresPublicationStatusReader 状态/事件投影演练、PostgresPublicationWriter publication/批量行/head/回滚演练、PostgresRelationRepository revision/UNCHANGED/变更/回滚演练、PostgresPublicationBackendFactory + OneClickPublisher PG 主流程 run/status/bulk/observation/outcome/head 清理演练、维护状态 DuckDB/PG 元数据读边界演练、维护窗口 preflight 硬门汇总、空 membership_entries 降级证据 |
