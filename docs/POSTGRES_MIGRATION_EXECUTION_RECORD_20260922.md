...
```text
MIGRATE_TO_PG consumers: 13/13 recorded
unmapped direct consumers: 0
current backend: DUCKDB (all 13)
target backend: POSTGRESQL
application switch: NOT_STARTED
acceptance: DEGRADED_PASS_PRECUTOVER_INVENTORY
```

盘点同时建立 `workbench_meta.migration_consumer_cutovers`，逐入口登记目标适配器、当前后端、回退合同和证据。13 个入口全部仍是 `NOT_MIGRATED`，所以不能把配置文件单独改成 PostgreSQL，也不能在当前阶段重启服务宣称完成切换。现有设计文档中的正式最终切换门仍未开始。
...
PGM-09 重新扫描并清理了消费者目录中的陈旧记录：当前静态目录为 106 个消费者，其中 `MIGRATE_TO_PG=13`、`OFFLINE_DUCKDB_ALLOWED=71`、`RETAIN_UNTIL_AUDIT_CLOSE=22`、`UNCLASSIFIED=0`。`TodayResearchBundleReader` 已移除 `duckdb.connect` 直连，`SourceFreezer`、结果对象协调器、切片协调器和维护状态读取也分别通过 `PublicationReadRepository`、`ResultObjectRepository`、`SliceRepository`、`OperationsMetadataReader` 边界收口；离线默认使用 DuckDB 适配器，切换阶段可注入 PostgreSQL。另将 `TurnoverEnrichmentService` 的内存 `read_parquet` 会话明确标记为 `OFFLINE_DUCKDB_ALLOWED`，因为它不打开 `market_research.duckdb`、不写库且只生成请求时指纹。上述变化减少的是静态误报和直连面，不代表 PostgreSQL 已成为线上主库。
...
下一步固定为 `PGM-09 / complete application adapter migration`：继续把 13 个入口按读边界、运维元数据边界、研究写入边界和主 API 注入分批收口，并重新执行只读 preflight；在所有硬门通过并经维护窗口授权前，不执行线上切换，不开始 Focus Tracker 业务表和算法实现。
