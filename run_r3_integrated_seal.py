...
def run():
 shadow=ROOT/f"reports/shadow/v2/{CUTOFF}";target=shadow/"integrated";pointer=ROOT/"reports/current/CURRENT_RELEASE.json";before=pointer.read_bytes();current=json.loads(before);baseline_run=current["latest_release"]["run_id"];baseline_identity=current["latest_release"]["computation_identity"]["sha256"];normal_universe_count=int(json.loads((ROOT/"reports/phase1/PHASE1_FINAL_RECEIPT.json").read_text("utf8"))["normal_universe_count"]);receipts={k:json.loads((shadow/v).read_text("utf8")) for k,v in RECEIPTS.items()}
 chain={k:{"status":v["final_status"],"errors":v.get("errors",[]),"path":RECEIPTS[k],"sha256":sha(shadow/RECEIPTS[k]),"baseline":v.get("v1_baseline_run_id",BASE),"v1_identity":v.get("v1_computation_identity_sha256",V1ID)} for k,v in receipts.items()};binding_checks,receipt_artifact_hash_pass=receipt_bindings(ROOT,shadow,receipts);chain_pass=all(v["status"]=="PASS" and not v["errors"] and v["baseline"]==baseline_run and v["v1_identity"]==baseline_identity for v in chain.values()) and receipt_artifact_hash_pass
