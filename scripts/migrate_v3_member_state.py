"""Migrate member_state to V3 result-object rows.

Read-only mode audits the legacy member-state slices. ``--apply`` applies the
versioned schema migration and imports every member-state slice in one database
...
from workbench_analysis.member_state import migrate_member_state_slice
...
def _audit(connection: duckdb.DuckDBPyConnection) -> dict[str, object]:
    ...
    if "analysis_slices" not in tables or "sector_member_state_daily" not in tables:
        return {"member_state_slice_count": 0, "legacy_row_count": 0, "status": "SCHEMA_INCOMPLETE"}
    slice_count, declared_row_count = connection.execute(
        "SELECT count(*), coalesce(sum(row_count), 0) FROM analysis_slices WHERE domain='member_state'"
    ).fetchone()
    actual_row_count = connection.execute(
        """
        SELECT count(*) FROM sector_member_state_daily legacy
        JOIN analysis_slices slices USING (slice_id)
        WHERE slices.domain='member_state'
        """
    ).fetchone()[0]
    result = {
        "member_state_slice_count": int(slice_count),
        "legacy_declared_row_count": int(declared_row_count),
        "legacy_actual_row_count": int(actual_row_count),
    }
    if "member_state_result_rows" in tables:
        result.update({
            "result_row_count": connection.execute("SELECT count(*) FROM member_state_result_rows").fetchone()[0],
            "result_object_count": connection.execute("SELECT count(*) FROM analysis_result_objects WHERE domain='member_state'").fetchone()[0],
            "member_state_binding_count": connection.execute("""
                SELECT count(*) FROM analysis_slice_result_bindings b
                JOIN analysis_slices s USING (slice_id)
                WHERE s.domain='member_state'
            """).fetchone()[0],
            "view_row_count": connection.execute("SELECT count(*) FROM member_state_result_daily").fetchone()[0],
        })
    ...
    with duckdb.connect(str(database), read_only=not args.apply) as connection:
        ...
        if args.apply:
            slice_ids = [row[0] for row in connection.execute(
                "SELECT slice_id FROM analysis_slices WHERE domain='member_state' ORDER BY slice_id"
            ).fetchall()]
            connection.execute("BEGIN TRANSACTION")
            try:
                imported = [migrate_member_state_slice(connection, slice_id) for slice_id in slice_ids]
