...
def main():
 source=json.loads((ROOT/'reports/p12_02/three_day_factor_replay.json').read_text(encoding='utf-8'));targets={x['date']:x['diagnostic_launch_security_ids'] for x in source['dates']};events=defaultdict(list)
 ...
 for t,ids in targets.items():
  ...
  for sid in ids:
   for h in H:
    ...
    outcomes.append({'signal_date':t,'security_id':sid,'horizon':h,'end_date':e,'anchor':e,'status':'OBSERVED','partition':part,'fret':fret,'mfe':mfe,'mae':mae});status['OBSERVED']+=1;values[h].append(fret)
  summary[t]={'partition':part,'signals':len(ids),'status_counts':dict(status),'median_fret':{str(h):sorted(v)[len(v)//2] if v else None for h,v in values.items()}}
