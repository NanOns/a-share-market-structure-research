...
def build_probe_manifest(*, evaluation_basis="HISTORICAL_RECONSTRUCTED",
    ...
    with PostgresRepository(dsn=_dsn()) as repo:
        ...
        repo.connection.rollback()
    stock_ids = {row.key.entity_id for row in accepted.rows
    ...
    resolved = resolve_baskets(repository=None, plan=planned,
                               contemporary={item.sector_id: item for item in baskets})
