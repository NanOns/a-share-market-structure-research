"""P12-13 bounded 170-session historical materialization."""
...
DATASET_ROOT = ROOT / "data/research_history_v3_3_170"
RECEIPT = ROOT / "reports/p12_13/p12_13_170_stage_gate.json"
STAGE_CONTRACT = "P12-13_170_SESSION_MATERIALIZATION_V1"
DATASET_CONTRACT = "TODAY_RESEARCH_HISTORY_BASE_170_V3_3_RECONSTRUCTED_01"
...
def main() -> None:
    ...
    with duckdb.connect(str(DB), read_only=True) as connection:
        ...
        if len(targets) != PILOT_DAYS:
            raise RuntimeError("P12_13_170_INSUFFICIENT_TARGET_SESSIONS")
        first_index = sessions.index(targets[0]) - 21
        if first_index < 0:
            raise RuntimeError("P12_13_170_INSUFFICIENT_WARMUP")
    ...
    checks = {
        "one_hundred_seventy_sessions": len(targets) == PILOT_DAYS,
    ...
    receipt = {
        "stage": "P12-13_170_SESSION_MATERIALIZATION", "stage_contract": STAGE_CONTRACT,
        "acceptance": acceptance, "acceptance_scope": "170-session reconstructed base facts and sector CURRENT materialization; PIT/effect claims excluded",
        ...
        "next_stage": "P12-14_OPTIONAL_TURNOVER_SOURCE" if acceptance == "FULL_PASS" else "P12-13_170_REPAIR",
    }
