...
def main():
 ...
 checks={'phase0_and_p12_02_accepted':json.loads((ROOT/'reports/p12_02/p12_02_stage_gate.json').read_text(encoding='utf-8'))['acceptance_result']=='DEGRADED_PASS','full_funnel_reconciles':total>0 and all(sum(v.values())==total for v in counts.values()),'launch_has_real_true_population':counts['launch'].get('True',0)>0,'setup_watch_has_real_true_population':counts['setup_watch'].get('True',0)>0,'stock_only_continue_preserved':counts['trend_continue_stock_only'].get('True',0)>0,'unsupported_continue_not_promoted':counts['trend_continue'].get('True',0)==0,'missing_pit_not_promoted':counts['pullback'].get('True',0)==0 and counts['recovery_turn'].get('True',0)==0}
