...
def main():
 before=(DB.stat().st_size,DB.stat().st_mtime_ns);first=read_active(POINTER);digest=first['output_digest'];second=read_active(POINTER);after=(DB.stat().st_size,DB.stat().st_mtime_ns);manifest=validate_bundle(Path(second['bundle_path']));rows=json.loads((Path(second['bundle_path'])/'results.json').read_text(encoding='utf-8'))
 checks={'p12_05_accepted':json.loads((ROOT/'reports/p12_05/p12_05_stage_gate.json').read_text(encoding='utf-8'))['acceptance_result']=='DEGRADED_PASS','complete_bundle_readable':len(manifest['files'])==2 and len(rows)>0,'repeat_read_is_stable':digest==second['output_digest'],'production_database_unchanged':before==after,'pointer_identity_complete':all(second['identity'].get(k) for k in ('publication_id','snapshot_id','membership_snapshot_id','research_run_id','parameter_hash','dependency_lock_hash','trade_date')),'verification_is_read_only':True}
