...
from workbench_db.operations_metadata import DuckDBOperationsMetadataReader, OperationsMetadataReader


class MaintenanceService:
    """Serialized M5 operations for the local workbench service."""

    def __init__(self, root, database_path=None, *, metadata_reader: OperationsMetadataReader | None = None):
        self.root=Path(root).resolve()
        self.database_path=Path(database_path).resolve() if database_path else DuckDBOperationsMetadataReader(self.root).database_path
        ...
        self.metadata_reader=metadata_reader or DuckDBOperationsMetadataReader(self.root, self.database_path)

    def status(self):
        counts=self.metadata_reader.metadata_counts()
        active=counts.get("active_jobs", counts.get("jobs", 0))
        objects=counts.get("storage_objects", 0)
        backups=counts.get("backup_catalog", 0)
