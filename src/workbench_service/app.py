 ...
 def market_cycle(self,p,days=60,metrics='',basis='AUTO',trade_date=None):
  ...
  with self._con() as c:
   ...
   if 'new_high' in requested or not requested:
    rows=c.execute("select cast(e.trade_date as varchar),h.\"window\",h.new_high from analysis_snapshot_entries e join stock_high_daily h on h.slice_id=e.slice_id and h.trade_date=e.trade_date where e.snapshot_id=? and e.domain='high' and e.trade_date in ("+placeholders+")",[selected['snapshot_id'],*dates]).fetchall()
   ...
   if 'queues' in requested or not requested:
    rows=c.execute("select cast(e.trade_date as varchar),h.security_id,h.queue_name,h.hit from analysis_snapshot_entries e join historical_structure_daily h on h.slice_id=e.slice_id and h.trade_date=e.trade_date where e.snapshot_id=? and e.domain='structure' and e.trade_date in ("+placeholders+")",[selected['snapshot_id'],*dates]).fetchall()
