...
class TodayResearchBundleReader:
 def __init__(self, root: Path, pointer: Path | None = None):
  self.root=Path(root).resolve();self.pointer=Path(pointer) if pointer else self.root/"data/current/ACTIVE_RESEARCH_BUNDLE_V3_3.json"
 ...
 def list(self,*,page:Any=1,page_size:Any=20,category:str="",selection_mode:str="",q:str="")->dict[str,Any]:
  ...
  filtered=[row for row in rows if (not category or category in row.get("matched_categories",[])) and (not selection_mode or row.get("selection_mode")==selection_mode) and (not query or query in str(row.get("security_id","")).upper())]
 ...
 def detail(self,security_id:str)->dict[str,Any]:
  try:active,rows=self._load()
  except (ResearchBundleError,OSError,ValueError,KeyError,json.JSONDecodeError) as exc:return self._unavailable(str(exc) or type(exc).__name__)
  wanted=str(security_id or "").strip().upper();item=next((row for row in rows if str(row.get("security_id","")).upper()==wanted),None)
